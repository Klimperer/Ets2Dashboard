...
/**
 * @brief Memory mapped file to share telemetry data with other processes.
 */
MemoryMappedFile* _memoryMappedFile;

/**
 * @brief Struct containing telemetry data that shall be stored in the memory mapped file.
 */
MappedTelemetryData _mappedData;
...
bool init_log(void)
{
    ...
    fopen_s(&log_file, "telemetry.log", "wt");
    if (!log_file) {
        return false;
    }
...
// Update of memory mapped file
void UpdateMappedFile()
{
    _memoryMappedFile->UpdateMappedFile(_mappedData);
}
...
SCSAPI_VOID telemetry_pause(const scs_event_t event, const void *const UNUSED(event_info), const scs_context_t UNUSED(context))
{
    ...
    _mappedData.gamePaused = output_paused;
    UpdateMappedFile();
}
...
SCSAPI_VOID telemetry_store_float(const scs_string_t name, const scs_u32_t index, const scs_value_t *const value, const scs_context_t context)
{
    ...
    *static_cast<float *>(context) = value->value_float.value;
    ...
    {
        _mappedData.speed = value->value_float.value;
    }
}
...
SCSAPI_RESULT scs_telemetry_init(const scs_u32_t version, const scs_telemetry_init_params_t *const params)
{
    ...
    version_params->register_for_channel(SCS_TELEMETRY_TRUCK_CHANNEL_speed, SCS_U32_NIL, SCS_VALUE_TYPE_float, SCS_TELEMETRY_CHANNEL_FLAG_none, telemetry_store_float, &_mappedData.speed);
    ...
    _memoryMappedFile = new MemoryMappedFile();

    if (_memoryMappedFile->IsFileMapped())
    {
        log_line("Memory file has been mapped.");
        _mappedData.gamePaused = output_paused;
        UpdateMappedFile();
    }
    else
    {
        log_line("Memory file has NOT been mapped.");
    }
